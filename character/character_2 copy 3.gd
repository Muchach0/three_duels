...
func _physics_process(delta: float) -> void:
    if _is_player():
        ...
        _physics_process_enemy_idle(delta)
...
func _physics_process_enemy_idle(delta: float) -> void:
...
func attack() -> void:
    if not _is_player() or _is_defeated or _is_blocking:
        return
...
func _set_blocking(blocking: bool) -> void:
    blocking = blocking and _is_player() and not _is_defeated
    var changed := _is_blocking != blocking

    _is_blocking = blocking

    if changed and _is_blocking:
        _cancel_attack()
    ...
    animation_tree.set(
        ...
        1.0 if _is_blocking else 0.0
    )


func receive_hit(attacker: Node, hit_data: Dictionary) -> void:
    if not _can_receive_hit(attacker, hit_data):
        return

    EventBus.combat_hit_detected.emit(attacker, self, hit_data)
...
func _setup_combat() -> void:
    ...
    add_to_group("combat_character")
    if _is_player():
        add_to_group("player_character")
    elif _is_enemy():
        add_to_group("enemy_character")

    hurtbox.set_meta("combat_owner", self)
    ...
    _emit_combat_stats_changed.call_deferred()
...
func _update_attack_window(_delta: float) -> void:
...
func _set_weapon_hitbox_active(active: bool) -> void:
    _weapon_hitbox_active = active and _is_player() and not _is_defeated
...
func _try_apply_weapon_hit(area: Area3D) -> void:
    ...
    defender.receive_hit(self, _build_hit_data(defender))
...
func _can_attack(defender: Node) -> bool:
    if not _is_player() or defender == self:
        return false
    if not defender.has_method("is_enemy_character") or not defender.is_enemy_character():
        return false
...
func _can_receive_hit(attacker: Node, _hit_data: Dictionary) -> bool:
    if _is_defeated or not _is_enemy() or attacker == self:
        return false
    if attacker == null or not attacker.has_method("is_player_character"):
        return false
    return attacker.is_player_character()
...
func _apply_health_damage(amount: float, hit_data: Dictionary) -> void:
    ...
    EventBus.combat_damage_applied.emit(self, hit_data)
    _emit_combat_stats_changed()

    if health <= 0.0 and not _is_defeated:
        _is_defeated = true
        _set_blocking(false)
        _set_weapon_hitbox_active(false)
        _play_death()
        EventBus.combat_character_defeated.emit(self)
    else:
        _play_hit_react()


func _emit_combat_stats_changed() -> void:
    EventBus.combat_stats_changed.emit(self, health, max_health, guard, max_guard)


func _play_hit_react() -> void:
    if animation_tree == null or not animation_tree.active or _hit_react_one_shot == null or _is_defeated:
        return
...
func _setup_weapon_hitbox() -> void:
    ...
    weapon_hitbox.set_meta("combat_owner", self)
...
func _set_weapon_hitbox(hitbox: Area3D) -> void:
    _clear_weapon_hitbox()
    if hitbox == null:
...
func _clear_weapon_hitbox() -> void:
    _set_weapon_hitbox_active(false)
    weapon_hitbox = null
    weapon_hitbox_shape = null


func _find_weapon_hitbox(node: Node) -> Area3D:
    if node is Area3D and node.name == "WeaponHitbox":
        return node as Area3D

    for child in node.get_children():
        var hitbox := _find_weapon_hitbox(child)
        if hitbox != null:
            return hitbox
...
func _equip_initial_player_weapon() -> void:
    var definitions := _find_props_in_slot(RIGHT_HAND_SLOT)
...
func equip_prop(definition: CharacterPropDefinition) -> void:
    ...
    if definition.slot == RIGHT_HAND_SLOT:
        _set_weapon_hitbox(_find_weapon_hitbox(prop_instance))
